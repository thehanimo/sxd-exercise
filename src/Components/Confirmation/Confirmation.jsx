...
function Confirmation({appMachineService}) {
  ...
  if (submitted) return (
    ...
    <div className='form-container'>
      <h1 style={{marginBottom: 48, textAlign: "center"}}>Your submission has been recorded</h1>
      <button className='button' style={{marginTop: 16, alignSelf: "center"}} onClick={()=>send({ type: 'RESET' })}>Start over</button>
      </div>
      </div>
  )
    ...
    <div className='bg'>
      <div className='form-container'>
        ...
        <div className='button-group'>
          ...
          <button className='button' style={{marginTop: 16}} onClick={submit}>Confirm</button>
        </div>
      </div>
    </div>
  );
